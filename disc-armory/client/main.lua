...
function showStoredWeapons()
    ...
    function(weapons)
        ...
        for k, v in pairs(weapons) do
            table.insert(w, {label = ESX.GetWeaponLabel(v.weapon) .. (' <span style="color:green;">x%s</span>'):format(v.count), action = function() takeOutWeapon(v.weapon) end})
        end
...
function showBuyMenu()
    ...
    for k, v in pairs(currentArmory.weapons) do
        table.insert(weapons, {label = ESX.GetWeaponLabel(v.name) .. (' <span style="color:green;">$%s</span>'):format(v.price), action = 
        function() 
...
function showStoringMenu()
    ...
    for i=1, #Config.Weapons, 1 do
        ...
        if HasPedGotWeapon(playerPed,  weaponHash,  false) then
            table.insert(weapons, {label = ESX.GetWeaponLabel(weapon), action = function() putWeapon(weapon) end})
        end
    end
    local menu = {
        name = "store_weapons",
